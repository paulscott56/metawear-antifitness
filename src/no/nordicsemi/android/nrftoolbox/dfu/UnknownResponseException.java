...
public class UnknownResponseException extends Exception {
	...
	public UnknownResponseException(final String message,
			final byte[] response, final int expectedOpCode) {
		super(message);
	...
	public String getMessage() {
		return String.format("%s (response: %s, expected: 0x10%02X..)",
				super.getMessage(), bytesToHex(mResponse, 0, mResponse.length),
				mExpectedOpCode);
	}

	public static String bytesToHex(final byte[] bytes, final int start,
			final int length) {
