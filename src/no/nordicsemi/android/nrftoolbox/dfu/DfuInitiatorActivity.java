...
/**
 * The activity is started only by a remote connected computer using ADB. It
 * shows a list of DFU-supported devices in range and allows user to select
 * target device. The HEX file will be uploaded to selected device using
 * {@link DfuService}.
 */
public class DfuInitiatorActivity extends FragmentActivity implements
	...
	protected void onCreate(final Bundle savedInstanceState) {
		...
		if (savedInstanceState == null) {
			final ScannerFragment fragment = ScannerFragment.getInstance(this,
					DfuService.DFU_SERVICE_UUID, true);
	...
	public void onDeviceSelected(final BluetoothDevice device, final String name) {
		final Intent intent = getIntent();
		final String overwritenName = intent
				.getStringExtra(DfuService.EXTRA_DEVICE_NAME);
